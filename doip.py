...
from peizhizi import peizhizi_map
zip_path = "/data/a.zip"
...
class DoipClient:
    ...
    def set_network(self,net= "enx207bd51a13cc"):
        execute(f"sudo ip link add link {net} name mgbe3_0.2 type vlan id 2 >/dev/null 2>&1 || true")
        execute(f"sudo ip link set mgbe3_0.2 type vlan egress 0:2 1:2 2:2 3:2 4:2 5:2 6:2 7:2")
        execute(f"sudo ip address add 172.16.2.58/16 dev mgbe3_0.2 >/dev/null 2>&1 || true")
        execute(f"sudo ip link set dev mgbe3_0.2 address 02:47:57:4d:00:58")
        execute(f"sudo ip link set dev mgbe3_0.2 up")
